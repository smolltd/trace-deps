...
const sortObj = (obj) =>
  uniq(Object.keys(obj)).reduce((memo, key) => {
    memo[key] = obj[key];
    return memo;
  }, {});
const toPosixPath = (file) =>
  !file ? file : path.normalize(file.replace(/\\/g, "/"));

// The dependency file name matches a configuration package prefix.
// Only allow:
// - exact match
// - match up to file separator
const matchesPkgPrefix = (depName) => (pkg) =>
  depName === pkg || depName.startsWith(`${pkg}/`);

// Convert all keys to posix paths.
const normalizeExtraImports = (extraImports) =>
  Object.entries(extraImports).reduce((memo, [key, vals]) => {
...
const _recurseDeps = async ({
  ...
  for (const depPath of depPaths) {
    const realDepPath = await fs.promises.realpath(depPath);

    if (!_tracedDepPaths.has(realDepPath)) {
      // Mark encountered to avoid future recursion.
      _tracedDepPaths.add(depPath);
      _tracedDepPaths.add(realDepPath);

      // Short-circuit: Is it a known non-traceable extension?
      if (NOT_JS_EXTS.has(path.extname(depPath))) {
        continue;
      }
...
const traceFile = async ({
  ...
  console.log("[traceFile]", { srcPath, realPath });
  ...
  const src = await readFile(srcPath)
    ...
    .catch((err) => {
      ...
      throw err.code === "ENOENT"
        ? new Error(`Could not find source file: ${srcPath}`)
        : err;
    });
  ...
  const addMisses = (vals = []) => {
    ...
    results.misses[fullSrcPath] = (results.misses[fullSrcPath] || []).concat(
      vals
    );
  };
  ...
  const depObjs = await Promise.all(
        ...
        resolve(depName, {
          ...
          .catch((err) => {
            ...
            if (err.code === "MODULE_NOT_FOUND") {
              ...
              const isAllowed =
                srcPkg &&
                (allowMissing[srcPkg] || []).some(matchesPkgPrefix(depName));
              if (isAllowed) {
                return null;
              }
              ...
              if (!bailOnMissing) {
                if (dependencies.has(depName)) {
                  addMisses([
                    Object.assign({}, dependencies.get(depName), {
                      type: "static",
                    }),
                  ]);
                  return null;
                ...
                throw new Error(
                  `Unaccounted dependency in ${srcPath} for ${depName}: ${err}`
                );
              }
            }
  ...
  const allPkgPaths = depObjs.reduce(
  ...
  results.dependencies = uniq([].concat(allPkgPaths, recursed.dependencies));

  if (includeSourceMaps) {
    results.sourceMaps = uniq(
      [].concat(results.sourceMaps || [], recursed.sourceMaps || [])
    );
  }
...
const traceFiles = async ({
  ...
  console.log("[traceFiles]", { srcPaths });
  _extraImports = _extraImports || normalizeExtraImports(extraImports);
